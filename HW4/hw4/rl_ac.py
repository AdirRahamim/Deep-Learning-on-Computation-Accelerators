...
class AACPolicyNet(nn.Module):
    def __init__(self, in_features: int, out_actions: int, **kw):
        ...
        policy = []
        hidden_dims = [128 ,64]
        for idx, in_f, out_f in zip(range(len([in_features] + hidden_dims)), [in_features] + hidden_dims,
                                    hidden_dims + [out_actions]):
            policy.append(nn.Linear(in_f, out_f))
            if idx < len([in_features] + hidden_dims) - 1:
                policy.append(nn.PReLU())

        self.policy_net = nn.Sequential(*policy)

        value = []
        hidden_dims = [128,64]
        for idx, in_f, out_f in zip(range(len([in_features] + hidden_dims)), [in_features] + hidden_dims,
                                    hidden_dims + [1]):
            value.append(nn.Linear(in_f, out_f))
            if idx < len([in_features] + hidden_dims) - 1:
