...
class Episode(object):
    ...
    def calc_qvals(self, gamma: float) -> List[float]:
        ...
        for experirence in reversed(self.experiences):
            qvals.insert(0, experirence.reward + (0 if len(qvals) == 0 else gamma * qvals[0]))
        # ========================
        return qvals
...
class TrainBatch(object):
    ...
    def from_episodes(cls, episodes: Iterable[Episode], gamma=0.999):
        ...
        q_vals_tensor = torch.FloatTensor(q_vals)

        states_tensor = torch.stack(states)

        rewards_tensor = torch.FloatTensor(total_rewards)

        actions_tensor = torch.LongTensor(actions)

        train_batch = TrainBatch(states_tensor, actions_tensor, q_vals_tensor, rewards_tensor)
    ...
    def episode_batch_generator(self) -> Iterator[Tuple[Episode]]:
        ...
        while True:
            ...
            while True:
                ...
                if episode_experiences[-1].is_done:
                    break
