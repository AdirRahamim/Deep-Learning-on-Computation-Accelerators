...
class PolicyNet(nn.Module):
    def __init__(self, in_features: int, out_actions: int, **kw):
        ...
        net = []
        hidden_dims = [128,64]
        for idx, in_f, out_f in zip(range(len([in_features] +hidden_dims)), [in_features] +hidden_dims, hidden_dims+[out_actions]):
            net.append(nn.Linear(in_f, out_f))
            if idx < len([in_features] +hidden_dims)-1:
                net.append(nn.PReLU())

        self.policy_net = nn.Sequential(*net)
        # ========================
...
class PolicyAgent(object):
    ...
    def step(self) -> Experience:
        ...
        action_probs = self.current_action_distribution()
        ...
        experience = Experience(curr_state, action, reward, is_done)
        # ========================
        if is_done:
            self.reset()
        return experience
...
class VanillaPolicyGradientLoss(nn.Module):
    ...
    def _policy_loss(self, batch, action_scores, policy_weight):
        ...
        # ====== YOUR CODE: ======
        action_taken = batch.actions
        log_prob = torch.log_softmax(action_scores, dim=1)
        prob_taken = log_prob.gather(dim = 1, index=action_taken.view(-1,1)).squeeze(1)
        loss_p = (-1.0/len(batch)) * ((prob_taken * policy_weight)).sum(dim=-1)
        # ========================
        return loss_p
...
class ActionEntropyLoss(nn.Module):
    ...
    def forward(self, batch: TrainBatch, action_scores, **kw):
        ...
        # ====== YOUR CODE: ======
        log_prob = torch.log_softmax(action_scores, 1)
        prob_actions = torch.softmax(action_scores, 1)

        entropys = -1.0 * (prob_actions * log_prob).sum(1)
        avg_entropy = (1/len(batch)) * entropys.sum(0)
        norm_entropy = avg_entropy / self.calc_max_entropy(action_scores.shape[1])
        loss_e = -1.0 * norm_entropy
        # ========================
...
class PolicyTrainer(object):
    ...
    def train_batch(self, batch: TrainBatch):
        ...
        for loss_fn in self.loss_functions:
            loss_p, dict = loss_fn(batch, action_scores)
            total_loss += loss_p
