...
class Discriminator(nn.Module):
    def __init__(self, in_size):
        ...
        for idx, in_c, out_c, ker, stride, padding in zip(range(len(channel_list)), [in_size[0]] + channel_list, channel_list, kernel_list, stride_list, padding_list):
...
class Generator(nn.Module):
    def __init__(self, z_dim, featuremap_size=4, out_channels=3):
        ...
        for idx, in_c, out_c, kernel, stride, padding in zip(range(len(channel_list)), [z_dim]+channel_list, channel_list, kernel_list, stride_list, padding_list):
...
def save_checkpoint(gen_model, dsc_losses, gen_losses, checkpoint_file):
    ...
    # ====== YOUR CODE: ======
    if len(gen_losses) > 3:
        if dsc_losses[-1] > dsc_losses[-2] and dsc_losses[-1] > dsc_losses[-2]:
            saved = True
