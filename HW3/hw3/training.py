...
class Trainer(abc.ABC):
    ...
    def fit(self, dl_train: DataLoader, dl_test: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            curr_train_loss = sum(train_result.losses) / len(train_result.losses)
...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        num_correct = torch.sum(y == torch.argmax(output, dim=1))
        # ========================
...
class VAETrainer(Trainer):
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        self.model.train()
        x_out, mu,  log_sigma2 = self.model(x)
        loss, data_loss, _ = self.loss_fn(x, x_out, mu, log_sigma2)
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        # ========================
