...
def char_maps(text: str):
    ...
    idx_to_char = dict(zip(range(len(sorted_text)), sorted_text))
    char_to_idx = dict(zip(sorted_text, range(len(sorted_text))))
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int,
    ...
    # ====== YOUR CODE: ======
    num_of_seq = len(text) // seq_len
    embedding = chars_to_onehot(text, char_to_idx)
    last = (num_of_seq * seq_len - 1) if len(text) % seq_len == 0 else num_of_seq * seq_len
    samples = embedding[0:last].view(num_of_seq, seq_len, -1).to(device)

    indices = embedding[1:last + 1].nonzero()[:, 1]
    labels = indices.view(num_of_seq, seq_len).to(device)
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    with torch.no_grad():
        ...
        embedded_text = chars_to_onehot(start_sequence, char_to_idx).to(dtype=torch.float, device=device)
        hs = None

        for char in range(n_chars - len(start_sequence)):
            y, hs = model(embedded_text.unsqueeze(dim=0), hidden_state=hs)
            new_char_idx = torch.multinomial(hot_softmax(y[0, -1, :], dim=0, temperature=T), num_samples=1)
            out_text += idx_to_char[new_char_idx.item()]
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        idx = torch.arange(0, num_of_batches * self.batch_size).view(-1, num_of_batches).t().reshape(1, -1)[0].tolist()
        # ========================
        return iter(idx)
...
class MultilayerGRU(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, out_dim, n_layers, dropout=0):
        ...
        for i in range(n_layers):
            ...
            self.add_module(name=f'w_xz_{i}', module=w_xz)
            self.add_module(name=f'w_hz_{i}', module=w_hz)
            self.add_module(name=f'w_xr_{i}', module=w_xr)
            self.add_module(name=f'w_hr_{i}', module=w_hr)
            self.add_module(name=f'w_xg_{i}', module=w_xg)
            self.add_module(name=f'w_hg_{i}', module=w_hg)
            self.add_module(name=f'dropout_{i}', module=drop)

            self.layer_params.append((w_xz, w_hz, w_xr, w_hr, w_xg, w_hg, drop))

        #Output layer
        self.w_hy = nn.Linear(h_dim, out_dim, bias=True)
        self.add_module(name='out', module=self.w_hy)
        # ========================
    ...
    def forward(self, input: Tensor, hidden_state: Tensor = None):
        ...
        for i in range(seq_len):
            x = layer_input[:,i,:]
            for i , ((w_xz, w_hz, w_xr, w_hr, w_xg, w_hg, drop)) in enumerate(self.layer_params):
                h = layer_states[i]
