...
class VAE(nn.Module):
    ...
    def encode(self, x):
        ...
        #epsilon = torch.randn_like(log_sigma2) * (10 ^ -5)
        #z = mu + torch.exp(0.5 * log_sigma2) * epsilon
    ...
    def decode(self, z):
        ...
        # ====== YOUR CODE: ======
        h_hat = self.h(z)
        h_hat = h_hat.view(z.shape[0], *tuple(self.features_shape))
...
def vae_loss(x, xr, z_mu, z_log_sigma2, x_sigma2):
    ...
    kldiv_loss = (torch.sum(torch.exp(z_log_sigma2)) + torch.norm(z_mu)**2 - torch.sum(z_log_sigma2))/x.shape[0] -z_mu.shape[1]
