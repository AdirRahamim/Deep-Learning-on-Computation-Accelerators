...
class LinearClassifier(object):
    ...
    def __init__(self, n_features, n_classes, weight_std=0.001):
        ...
        self.weights = None
        # ====== YOUR CODE: ======
        weight = torch.empty(n_features, n_classes).normal_(mean=0, std=weight_std)
        self.weights = weight
        # ========================
    ...
    def evaluate_accuracy(y: Tensor, y_pred: Tensor):
        ...
        acc = None
        # ====== YOUR CODE: ======
        num_equal = (y == y_pred).sum().item()
        num_items = y.shape[0]
        acc = num_equal/num_items
        # ========================
    ...
    def train(self,
        ...
        for epoch_idx in range(max_epochs):
            ...
            for x_train, y_train in dl_train:
                ...
                grad = loss_fn.grad()
                regularization_term = self.weights * weight_decay
                self.weights = self.weights - learn_rate * (grad + regularization_term)
                total_loss += loss + 0.5*weight_decay*(torch.norm(self.weights))**2
                total_correct += self.evaluate_accuracy(y_train,y_pred)

            average_loss = total_loss / count
            average_accuracy = total_correct / count
            ...
            for x_valid, y_valid in dl_valid:
                ...
                total_loss += loss + 0.5 * weight_decay * (torch.norm(self.weights)) ** 2
                total_correct += self.evaluate_accuracy(y_valid, y_pred)

            average_loss = total_loss / count
            average_accuracy = total_correct / count
            valid_res.loss.append(average_loss)
            valid_res.accuracy.append(average_accuracy)
            # ========================
            print('.', end='')
    ...
    def weights_as_images(self, img_shape, has_bias=True):
        ...
        if has_bias:
            weights = weights[1:]
        w_images = weights.t().view((-1, *img_shape))
        # ========================
...
def hyperparams():
    hp = dict(weight_std=0.005, learn_rate=0.005, weight_decay=0.005)
