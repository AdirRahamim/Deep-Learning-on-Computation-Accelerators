...
class KNNClassifier(object):
    ...
    def predict(self, x_test: Tensor):
        ...
        for i in range(n_test):
            ...
            # ====== YOUR CODE: ======
            _ , topk_indices = torch.topk(dist_matrix[:,i], self.k, largest=False)
            # Find k most frequent label
            y_topk = self.y_train[topk_indices]
            y_pred[i] = torch.argmax(torch.bincount(y_topk))
...
def find_best_k(ds_train: Dataset, k_choices, num_folds):
    ...
    for i, k in enumerate(k_choices):
        ...
        for curr_fold in range(num_folds):
            test_indices = indices[curr_fold * fold_size : (curr_fold + 1) * fold_size]
            train_indices = indices[:curr_fold * fold_size] + indices[(curr_fold + 1) * fold_size:]

            test_sampler = torch.utils.data.sampler.SubsetRandomSampler(test_indices)
            train_sampler = torch.utils.data.sampler.SubsetRandomSampler(train_indices)

            dl_test = torch.utils.data.DataLoader(ds_train, sampler=test_sampler)
            ...
            temp_accu.append(accuracy(y_test,y_predict))
