...
class LinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        mat[0][0] = 0
        w_opt = np.linalg.inv(X.T @ X + self.reg_lambda * X.shape[0] * mat) @ (X.T @ y)
        # ========================
...
class BiasTrickTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: np.ndarray):
        ...
        xb = None
        # ====== YOUR CODE: ======
        ones = np.ones((X.shape[0],1), dtype=np.int)
        xb = np.hstack((ones,X))
        # ========================
...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    # ====== YOUR CODE: ======
    top_n_corr = df.corr()[target_feature].drop(target_feature).abs().nlargest(n).sort_values(ascending=False)
    top_n_features = list(top_n_corr.index)
    top_n_corr = list(top_n_corr.values)
    # ========================
...
def mse_score(y: np.ndarray, y_pred: np.ndarray):
    ...
    # ====== YOUR CODE: ======
    mse = np.sum(np.square(y-y_pred)) * (1/len(y))
    # ========================
    return mse
...
def r2_score(y: np.ndarray, y_pred: np.ndarray):
    ...
    # ====== YOUR CODE: ======
    numerator = mse_score(y,y_pred)*(len(y))
    y_mean = np.mean(y)
...
def cv_best_hyperparams(model: BaseEstimator, X, y, k_folds,
    ...
    # ====== YOUR CODE: ======
    kf = sklearn.model_selection.GroupKFold(k_folds)

    best_mse = np.Inf
    best_params = {"bostonfeaturestransformer__degree": 0, "linearregressor__reg_lambda": 0}

    for lam in lambda_range:
        for deg in degree_range:
            model.set_params(bostonfeaturestransformer__degree = deg, linearregressor__reg_lambda = lam)

            for train_index , test_index in kf.split(X,y,range(X.shape[0])):
                curr_model = model.fit(X[train_index], y[train_index])
                y_pred = model.predict(X[test_index])
                curr_mse = mse_score(y[test_index], y_pred)
                if float(curr_mse)<best_mse:
                    best_mse = curr_mse
